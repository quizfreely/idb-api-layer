...
export const idbApiLayer = {
    ...
    getStudysetsByIds: async function (ids: number[], resolveProps?: { terms?: boolean | TermResolveProps; termsCount?: boolean }) {
        ...
        if (resolveProps?.terms) {
            ...
            const countByStudysetId = new Map<number, number>();
            for (const term of allTerms) {
                countByStudysetId.set(
                    term.studysetId,
                    (countByStudysetId.get(term.studysetId) ?? 0) + 1
                );
            }
            for (const studyset of studysets) {
                if (studyset == null) continue;
    ...
    getReviewEventStatsByDay: async function ({
        ...
        const termIdsToLookup: (number | string)[] | null =
            termIds != null && termIds.length > 0
                ? termIds
                : studysetId != null
    ...
    recordPracticeTest: async function (practiceTest: any, getCloudStudysetIds?: (cloudTermIds: string[]) => Promise<(number | string)[]>) {
        return await db.transaction('rw', [db.practiceTests, db.practiceTestQuestions, db.termProgress, db.terms, db.reviewEvents, db.recentActivity], async () => {
            ...
            const involvedTermIds = new Set<number | string>();
            ...
            if (localInvolvedTermIds.length > 0) {
                const termIdsForLookup = localInvolvedTermIds.map(id => Number(id)) as number[];
            ...
            const cloudInvolvedTermIds = Array.from(involvedTermIds).filter(id => typeof id === 'string' && id.includes('-')) as string[];
    ...
    updatePracticeTestQuestion: async function (id: number, correct: boolean, userMarkedCorrect?: boolean) {
        return await db.transaction('rw', [db.practiceTests, db.practiceTestQuestions, db.termProgress, db.reviewEvents], async () => {
            ...
            // Update question
            const newData = { ...question.data } as any;
            if (question.type === "frq") {
    ...
    getPracticeTestsByTermId: async function (termId: number | string) {
        ...
        const questions = await db.practiceTestQuestions.bulkGet(questionIds as number[]);
        const ptIds = new Set<number>();
        questions.forEach(q => { if (q) ptIds.add(q.practiceTestId); });
    ...
    getMatchActivityById: async function (id: number, resolveProps?: MatchActivityResolveProps) {
        ...
        return await db.transaction('rw', [db.matchActivities, db.reviewEvents, db.termProgress, db.terms, db.recentActivity], async () => {
            ...
            for (const termId of termIds) {
                reviewEventsToInsert.push({
                    ...
                    reviewActivityType: "MATCH" as ReviewActivityType,
                    answeredString: null
                });
